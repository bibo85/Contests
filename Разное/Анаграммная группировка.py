...
#
# Формат ввода
# В первой строке записано число n —– количество строк. Далее в строку через пробел записаны n строк. n не превосходит
# 6000. Длина каждой строки не более 100 символов.
#
# Формат вывода
...
def get_hash(inp_str: str) -> int:
    hash_str = 0
    for s in inp_str:
        hash_str += (ord(s) * 3 * 15 % 1019) ** 3
    return hash_str


count_s: int = int(input())
...
for i in range(count_s):
    key = hash(strings[i])
    value: list = hash_map.get(key, [])
    value.append(i)
    hash_map[key] = value

for indexes_lst in sorted(hash_map.values()):
    print(*indexes_lst)
